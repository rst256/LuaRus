...
local decindent = {
  ['иначе'] = true, ['иначеесли'] = true, ['until'] = true, ['конец'] = true}
local incindent = {
  ['иначе'] = true, ['иначеесли'] = true, ['для'] = true, ['начало'] = true,
  ['если'] = true, ['repeat'] = true, ['пока'] = true}
local function isfndef(str)
  local l
  local s,e,cap,par = string.find(str, "функция%s+" .. funcdef .. "(%(.-%))")
  -- try to match without brackets now, but only at the beginning of the line
  if (not s) then
    s,e,cap = string.find(str, "^%s*функция%s+" .. funcdef)
  end
  -- try to match "foo = function()"
  if (not s) then
    s,e,cap,par = string.find(str, funcdef .. "=%s*функция%s*(%(.-%))")
  end
  if (s) then
    l = string.find(string.sub(str,1,s-1),"локал%s+$")
    cap = cap .. " " .. (par or "(?)")
  end
  return s,e,cap,l
end
...
local function ldoc(tx, typepatt)
  local varname = "([%wабвгдёежзиклмнопрстуфхчцьъыюя_АБВГДЁЕЖЗИКЛМНОПРСТУФХЧЦЬЪЫЮЯ]+)"
  -- <type> == ?string, ?|T1|T2
...
return {
  ...
  isdecindent = function(str)
    ...
    local term = (str:match("^%s*(%w+)%s*$")
      or str:match("^%s*(иначеесли)[%s%(]")
      or str:match("^%s*(until)[%s%(]")
      or str:match("^%s*(иначе)%f[%W]")
    )
    -- (1) 'end', 'elseif', 'else', 'until'
    local match = term and decindent[term]
    -- (2) 'end)', 'end}', 'end,', and 'end;'
    if not term then term, match = str:match("^%s*(конец)%s*([%)%}]*)%s*[,;]?") end
    -- endFoo could be captured as well; filter it out
    if term and str:match("^%s*(конец)%w") then term = nil end
    -- (3) '},', '};', '),' and ');'
  ...
  isincindent = function(str)
    ...
    local func = (isfndef(str) or str:match("%W+функция%s*%(")) and 1 or 0
    ...
    if (term == 'если' or term == 'иначеесли') and not str:match("%f[%wтогда]тогда%f[%W]")
    or (term == 'для') and not str:match("%S%s+начало%f[%W]")
    or (term == 'пока') and not str:match("%f[%wначало]начало%f[%W]")
    -- if this is a function definition, then don't increment the level
    or func == 1 then
      terminc = 0
    elseif not (term == 'если' or term == 'иначеесли') and str:match("%f[%wтогда]тогда%f[%W]")
    or not (term == 'для') and str:match("%S%s+начало%f[%W]")
    or not (term == 'пока') and str:match("%f[%w]начало%f[%W]") then
      terminc = 1
    end
    local _, opened = str:gsub("([%{%(])", "%1")
    local _, closed = str:gsub("([%}%)])", "%1")
    -- ended should only be used to negate term and func effects
    local anon = str:match("%W+функция%s*%(.+%Wконец%W")
    local ended = (terminc + func > 0) and (str:match("%W+конец%s*$") or anon) and 1 or 0

    return opened - closed + func + terminc - ended
  end,
  marksymbols = function(code, pos, vars)
    local PARSE = require 'lua_parser_loose_r'
    local LEX = require 'lua_lexer_loose_r'
    ...
    while (line <= endline) do
      ...
      elseif not iscomment[s] then
        ...
        typ = (typ and typ
          ...
          :gsub("%-%-.*",""))
        if (typ and (typ:match(",") or typ:match("%sили%s") or typ:match("%sи%s"))) then
          typ = nil
        end
  ...
  keywords = {
    [[и стоп начало иначе иначеесли конец для функция идина если in локал не или repeat возврат тогда until пока]],

    [[_G _VERSION _ENV ложь io.stderr io.stdin io.stdout нуль math.huge math.pi я истина]],

    [[утв collectgarbage dofile error getfenv getmetatable ipairs load loadfile loadstring
      module next pairs pcall печать rawequal rawget rawlen rawset require
    ...
    [[bit32.arshift bit32.band bit32.bnot bit32.bor bit32.btest bit32.bxor bit32.extract
      ...
      io.close io.flush io.input io.lines вв.открыть io.output io.popen io.read io.tmpfile io.type io.write
