...
function PARSE.parse_scope(lx, f, level)
  ...
  while true do
    ...
    or c.tag == 'Keyword' and (
       c[1] == 'стоп' or c[1] == 'идина' or c[1] == 'начало' or c[1] == 'пока' or
       c[1] == 'repeat' or c[1] == 'если' or c[1] == 'для' or c[1] == 'функция' and lx:peek().tag == 'Id' or
       c[1] == 'локал' or c[1] == ';' or c[1] == 'until' or c[1] == 'возврат' or c[1] == 'конец') or
       c.tag == 'Id' and
           (cprev.tag == 'Id' or
            cprev.tag == 'Keyword' and
               (cprev[1] == ']' or cprev[1] == ')' or cprev[1] == '}' or
                cprev[1] == '...' or cprev[1] == 'конец' or
                cprev[1] == 'истина' or cprev[1] == 'ложь' or
                cprev[1] == 'нуль') or
    ...
    then
      ...
      f('Statement', c[1], c.lineinfo,
        scopes[scope].inside_local or c[1] == 'локал' or c[1] == 'функция' or c[1] == 'конец')
    end
    ...
    if c.tag == 'Keyword' then
      ...
      if c[1] == 'локал' and lx:peek().tag == 'Keyword' and lx:peek()[1] == 'функция' then
        -- local function
        local c = lx:next(); assert(c[1] == 'функция')
        if lx:peek().tag == 'Id' then
          c = lx:next()
          f('Var', c[1], c.lineinfo, true)
          if lx:peek()[1] == '(' then parse_function_list(nil, c[1], c.lineinfo) end
        end
      elseif c[1] == 'функция' then
      ...
      elseif c[1] == 'локал' and lx:peek().tag == 'Id' then
      ...
      elseif c[1] == 'для' and lx:peek().tag == 'Id' then
        c = lx:next()
      ...
      elseif c[1] == 'идина' and lx:peek().tag == 'Id' then
        lx:next()
      elseif c[1] == 'начало' then
        scope_begin('начало', c.lineinfo)
        -- note: do/while/for statement scopes all begin at 'начало'.
      elseif c[1] == 'repeat' or c[1] == 'тогда' then
        scope_begin(c[1], c.lineinfo)
      elseif c[1] == 'конец' or c[1] == 'иначеесли' then
        scope_end(c[1], c.lineinfo)
      elseif c[1] == 'иначе' then
...
function PARSE.parse_scope_resolve(lx, f, vars)
  ...
  local function newscope(vars, opt, lineinfo)
    local newvars = opt=='начало' and vars[INSIDE] or {}
