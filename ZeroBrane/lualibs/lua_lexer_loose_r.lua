...
local sym = newset("abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZабвгдёежзиклмнопрстуфхчцьъыюя_АБВГДЁЕЖЗИКЛМНОПРСТУФХЧЦЬЪЫЮЯ_")
local dig = newset('0123456789')
local name = "([_A-Za-zабвгдёежзиклмнопрстуфхчцьъыюя_АБВГДЁЕЖЗИКЛМНОПРСТУФХЧЦЬЪЫЮЯ][абвгдёежзиклмнопрстуфхчцьъыюя_АБВГДЁЕЖЗИКЛМНОПРСТУФХЧЦЬЪЫЮЯ_A-Za-z0-9]*)"
local op = newset('=~<>.+-*/%^#=<>;:,.{}[]()')
...
local is_keyword = qws[[
  и стоп начало иначе иначеесли конец ложь для функция если
  in локал нуль не или repeat возврат
  тогда истина until пока идина]]
