локал док = вв.открой([[c:\dev\luaRu5.3.4\lua-5.3.4\doc\contents.html]]):читай"*a"
док = док:gsub('"manual%.html#', '"manual_ru.html#')
...
локал функция найти_англ_аналог(фн, окр)
	...
	для н, т из пар(окр или _ОКР) начало
		если т==фн и н:найти"^[%w_]+$" тогда
			возврат н
		конец
	конец
конец

локал функция найти_англ_аналог2(фн)
	-- если фн==вв возврат "io" конец
	локал к
	для н, т из пар(фн) начало
		если н:найти"^[%r_]+$" тогда
			к = найти_англ_аналог(т, фн)
			если к тогда стоп конец
		конец
	конец
	если не к тогда возврат конец
	локал зр = фн[к]
	для н, т из пар(_ОКР) начало
		если тип(т)=="таблица" и т[к]==зр и н:найти"^[%w_]+$" тогда
			возврат н
		конец
	конец
конец
...
для н, т из пар(_ОКР)
	если н~="_ОКР" и н~="_G" и тип(т)=="таблица" и н:найти"^[%r_]" 
		локал аам = найти_англ_аналог2(т)
		печать(н, аам)
		для н2, т2 из пар(т) начало
			если н2:найти"^[%r_]"  		
				локал аа = найти_англ_аналог(т2, т)
				если аа и аам 
					док = док:gsub('([^%w_%-"%.])'..аам.."%."..аа..'([^%w_%-"])', "%1"..н.."."..н2.."%2") 
					ман = ман:gsub('([^%w_%-"%.])'..аам.."%."..аа..'([^%w_%-"])', "%1"..н.."."..н2.."%2") 
				конец
				печать("", н2, аа) 
			конец
		конец
		если аам 
			док = док:gsub('([^%w_%-"%.])'..аам..'([^%w_%-"])', "%1"..н.."%2") 
			ман = ман:gsub('([^%w_%-"%.])'..аам..'([^%w_%-"])', "%1"..н.."%2") 
		конец		
	конец
конец
печать""
для н, т из пар(_ОКР) начало
	если тип(т)=="функция" и н:найти"^[%r_]" 
		локал аа = найти_англ_аналог(т, _G)
		печать(н, аа)
		если аа 
			док = док:gsub('([^%w_%-"%.])'..аа..'([^%w_%-"])', "%1"..н.."%2") 
			ман = ман:gsub('([^%w_%-"%.])'..аа..'([^%w_%-"])', "%1"..н.."%2") 
		конец
	конец
конец
вв.открой([[c:\dev\luaRu5.3.4\lua-5.3.4\doc\contents_ru.html]], "w+"):пиши(док)
вв.открой([[c:\dev\luaRu5.3.4\lua-5.3.4\doc\manual_ru.html]], "w+"):пиши(ман)
печать(затребуй, require)
ос.выход()
...
локал Тип = создать_метатип{
	вызов_поля = функция(я, имя, ...)
		...
		если член == нуль 
			ошибка()
		иесли член.род == "функция" 
			возврат код_вызова_функции(имя_класса.."__"..имя, я, ...)
		иесли член.род == "переменная" 
	...
	реальное_описание_типа = функция()
		...
		для н, св из pairs(члены_класса.симтаб) начало
			если св.род == "переменная"  свойства[н] = св.тип конец
		конец
...
функция требстр(итер, ток)
	...
	если с~=ток  ощибка("синт. ошибка: ожидается "..ток.." получен "..с) конец
конец
...
функция А(...)
	...
	возврат функция(итер)
		...
		для инд, элем из перечисли(альт)
			если элем(итер)  возврат истина конец
			строка.токитер_вернуть(итер, итер_поз)
		конец
		возврат ложь
	конец
...
функция С(стр)
	возврат функция(итер)
		локал л, с = итер()
		если л=="ws"  л, с = итер() конец
		если с==стр  возврат истина конец
		возврат ложь
	конец
конец
...
функция О(правило)
	возврат функция(итер)
		локал итер_поз = строка.токитер_запомнить(итер)
		если не правило(итер)  строка.токитер_вернуть(итер, итер_поз) конец
		возврат истина
	конец
конец
...
функция лексошибка(итер, сообщение)
	локал лин, кол = строка.токитер_позиция(итер)
	печать([[C:\dev\luaRu5.3.4\lua-5.3.4\src\test.lua:]]..(лин+454)..":"..кол..":",сообщение)
	ос.выход()
конец
...
--печать(64  	 000, 0x 44 ae, 0ш 44 ае, 0ш 44 АЕ, 66 000.000 7 e +  10, 66000.0007е+10, 66 000.000 7 Е +10)
печать"1\т2\н3\с4\в5"
-- ос.выход()
...
для н, т из пар(_ОКР)
	если н~="_ОКР" и н~="_G" и тип(т)=="таблица" и н:найти"^[%r_]" 
		печать(н)
		для н2, т2 из пар(т) начало
			 если н2:найти"^[%r_]"  печать("", н2) конец
		конец
	конец
конец
печать""
...
-- 		-- если рн  мт_англ[рн] = в иначе  конец
-- 	конец
...
s=[[<A HREF="manual.html#pdf-load">load</A><BR>
<A HREF="manual.html#pdf-loadfile">loadfile</A><BR>
<A HREF="manual.html#pdf-next">next</A><BR>
<A HREF="manual.html#pdf-pairs">pairs</A><BR>
<A HREF="manual.html#pdf-pcall">pcall</A><BR>
<A HREF="manual.html#pdf-ЯХзРвм">ЯХзРвм</A><BR>
<A HREF="manual.html#pdf-rawequal">rawequal</A><BR>
<A HREF="manual.html#pdf-rawget">rawget</A><BR>
<A HREF="manual.html#pdf-rawlen">rawlen</A><BR>
<A HREF="manual.html#pdf-rawset">rawset</A><BR>
]]
печать(s:gsub("[^%w_]([%w_]+)[^%w_]", "(%1)"))
ос.выход()
